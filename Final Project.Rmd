...
list.of.packages <- c("ggplot2", "tidyverse","magrittr","mice","MVA","ggpubr","reshape2","ggcorrplot","mclust")
...
```{r qqplot}
# tests for normality
names <- c("Weight", "Total Fat", "Saturated Fat", "Cholesterol", "Sodium", "Sugar",
           "Fiber", "Total Carbs","Protein","Vitamini A", "Vitamin C", "Calcium","Iron")
i = 1
par(mfrow=c(2,3))
for(variable in colnames(data.vals[1:6])){
  
  qqnorm(data[,variable],main = names[i])
...
par(mfrow=c(2,3))
for(variable in colnames(data.vals[7:12])){
...
par(mfrow=c(2,3))
for(variable in colnames(data.vals[13])){
...
d2 <- melt(x2)
par(mfrow = c(1,1))
g1 <- ggplot(d,aes(x=value, fill=variable)) + geom_density(alpha=0.25) + xlab("Standardized Value") +
  ylab("Density") + ggtitle("Density Plots")
g2 <- ggplot(d2,aes(x=value, fill=variable)) + geom_density(alpha=0.25)+ xlab("Standardized Value") +
  ylab("Density") 

d3 <- melt(x3)
d4 <- melt(x4)
g3 <- ggplot(d3,aes(x=value, fill=variable)) + geom_density(alpha=0.25)+ xlab("Standardized Value") +
  ylab("Density") 
g4 <- ggplot(d4,aes(x=value,fill=variable))+geom_density(alpha=0.25)+ xlab("Vitamin A (DV%)") +
  ylab("Density") 
...
Sugar and fiber, being types of carbs, have decently strong correlation with total carbs (r =0.70, r =0.58, respectively). Protein, being the third macronutrient after fat and carbs, has strong negative correlation with total carbs (r =-0.76). These observations made so far shouldn't be surprising and won't be the focus of the analysis despite some of the largest correlation values. However, it's important to note this relationship with those unfamiliar with macronutrient composition of foods. One could also takeaway to try and eat higher protein foods if they are trying to limit their sugar intake (correlation of sugar and protein is -0.57). 
...
bvbox(x,method="nonrobust",xlab="Vitamin A (DV%)", ylab="Iron (DV%)", type="n")
text(x[c(20,36),"vitamin.a"],x[c(20,36),"iron"],labels=data$food.item[c(20,36)],col="black")
points(x = x[-c(20,36),'vitamin.a'], y= x[-c(20,36),'iron'])
...
plot(x, xlab="Vitamin A (DV%)", ylab="Calcium (DV%)", type="n")
text(x[c(31,10,19,7,20,36),'vitamin.a'],x[c(31,10,19,7,20,36),"calcium"],labels=data$food.item[c(31,10,19,7,20,36)],cex=1)
points(x = x[-c(31,10,19,7,20,36),'vitamin.a'], y= x[-c(31,10,19,7,20,36),'calcium'])
```
...
bvbox(x,method="robust", xlab="Iron (DV%)", ylab = "Calcium (DV%)", type="n")
text(x[c(31,10,19,7,32,36),"iron"],x[c(31,10,19,7,32,36),"calcium"],labels=data$food.item[c(31,10,19,7,32,36)],cex=1)
points(x = x[-c(31,10,19,7,32,36),'iron'], y= x[-c(31,10,19,7,32,36),'calcium'])
...
bvbox(x,method="robust", xlab = "Vitamin C (DV%)", ylab = "Weight (g)", type="n")
...
points(x = x[-c(6,15,17,31,35,36,37,41),'vitamin.c'], y= x[-c(6,15,17,31,35,36,37,41),'weight'])
...
stars(data[-c(36),c("vitamin.a","vitamin.c","iron","calcium")],labels=data$food.item[-36], main="Starplots")
...
stars(data.dv[,c("vitamin.a","vitamin.c","iron","calcium")],labels = data.dv$food.item, main = "Starplots")
...
stars(data.dv.fv[,c("vitamin.a","vitamin.c","iron","calcium")],labels = data.dv.fv$food.item, main="Fruit and Vegetable Starplots")
...
colors <- c("blue","purple","brown","black","red","darkgreen")
# plot solution 
x <- cmd$points[,1]
y <- cmd$points[,2]
plot(x, y, xlab="Coordinate 1", ylab="Coordinate 2", 
  main="Metric Multidimensional Scaling",	type="n")
...
legend("bottomright", legend=levels(data$group), pch=16, col=colors)
```
...
plot(PC1scores,PC2scores, xlab = "PC 1 (50%)", ylab = "PC 2 (18%)", main="PCA of Macronutrients", type = "n")
gp <- data$group == "vegetable"
...
```
**Table 4**
```{r pca vit and min loadings}
print(pca$loadings)
```
**Figure 16**

```{r pca vitamins and minerals graph}
PC1scores <- pca$scores[,1]
PC2scores <- pca$scores[,2]
plot(PC1scores,PC2scores, main = "PCA of Vitamins and Minerals", xlab = "PC1 (55%)", ylab = "PC2 (23%)",type = 'n')
gp <- data$group == "vegetable"
text(PC1scores[gp],PC2scores[gp],labels=data$food.item[gp],col="darkgreen")
gp <- data$group == "fruit"
text(PC1scores[gp],PC2scores[gp],labels=data$food.item[gp],col="purple")
...
# biplot(pca, xlabs=data$food.item, xlim=c(-10,10), ylim=c(-10,10))
# gp <- data$group == "vegetable"
# text(PC1scores[gp],PC2scores[gp],labels=data$food.item[gp],col="darkgreen")
# gp <- data$group == "fruit"
# text(PC1scores[gp],PC2scores[gp],labels=data$food.item[gp],col="purple")
...
```{r pca v m outliers removed biplot}

biplot(pca,xlabs=data$food.item[-c(6,15, 17, 20, 31, 35, 36, 41)], xlab = "PC1 (69%)", ylab = "PC2 (23%)")
# PC1scores <- pca$scores[,1]
# PC2scores <- pca$scores[,2]
...
summary(pca)
```
**Table 7:** loadings for first three principal components.
...
eigvals <- pca$sdev^2
k <- length(eigvals)
plot(1:k,eigvals,type="b",xlab="Principal Components",ylab=expression(lambda[i]), main = "Scree Plot")
```
...
PC1scores <- pca$scores[,1]
PC2scores <- pca$scores[,2]
plot(PC1scores,PC2scores,  type = 'n', main = 'PCA',xlab = "PC1 (36% of variance)",ylab = 'PC2 (21% of variance)')
gp <- data$group == "vegetable"
text(PC1scores[gp],PC2scores[gp],labels=data$food.item[gp],col="darkgreen")
gp <- data$group == "fruit"
text(PC1scores[gp],PC2scores[gp],labels=data$food.item[gp],col="purple")
...
gp <- data$group == "legume/nut"
text(PC1scores[gp],PC2scores[gp],labels=data$food.item[gp],col="black")
legend("topleft", legend=levels(data$group), pch=16, col=colors)
...
plot(PC1scores,PC3scores,type = 'n', main = "PCA", xlab = "PC1 (36% of variance)", ylab = 'PC3 (9% of variance)')
gp <- data$group == "vegetable"
...
PC2scores <- pca$scores[,2]
PC3scores <- pca$scores[,3]
plot(PC2scores,PC3scores, type = 'n', main = 'PCA',xlab = "PC2 (21% of variance)", ylab = "PC3 (9% of varinace)")
gp <- data$group == "vegetable"
...
legend("topright", legend=levels(data$group), pch=16, col=colors)
...
colors <- c("brown","red","turquoise","darkgreen","purple", "black")
...
text(x[gp],y[gp],labels=data$food.item[gp],col="red")
gp <- mclass == 3
text(x[gp],y[gp],labels=data$food.item[gp],col="turquoise")
gp <- mclass == 4
text(x[gp],y[gp],labels=data$food.item[gp],col="darkgreen")
gp <- mclass == 5
text(x[gp],y[gp],labels=data$food.item[gp],col="purple")
gp <- mclass == 6
text(x[gp],y[gp],labels=data$food.item[gp],col="black")
legend("bottomright", legend=levels(as.factor(mclass)), pch=16, col=colors, title = "Cluster")
